...
import { AiOutlineClose, AiTwotoneMinusCircle, AiTwotonePlusCircle } from 'react-icons/ai';

export default function Task({ addTodo, btnEvent, setBtn, setSearch }: { addTodo: Function; setBtn: Function; setSearch: any; btnEvent: boolean }) {
  type todoList = {
    tasks: {
      value: string;
    }[];
  ...
  return (
    <div className={`font-bold bg-[#60c0bf] border-2 border-black rounded-md px-10 p-5 w-full md:w-1/2 lg:w-full m-auto relative`}>
      <h1 className="text-3xl capitalize mb-3 text-center">tambah todo</h1>
      <form onSubmit={handleSubmit(onSubmit, () => setSearch(null))} className="flex flex-col gap-1">
        {fields.map((field, index) => (
          <>
            ...
            {errors.tasks && <span className="text-red-500 ">{errors.tasks[index]?.value?.message}</span>}
          </>
        ))}
      ...
      <button className={`flex lg:hidden absolute top-0 right-0 p-3 bg-slate-700/30 rounded-bl-md z-10`} onClick={() => setBtn(!btnEvent)}>
        <AiOutlineClose />
      </button>
    </div>
  );
