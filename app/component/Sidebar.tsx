...
=======
import { useEffect, useRef, useState } from 'react';
import { AiOutlineCheckCircle, AiOutlineFileDone, AiOutlineMenu, AiOutlineClockCircle } from 'react-icons/ai';
>>>>>>> dbcd66a1b67cf6154678b8bab05bc3bc849f08b8

export default function Sidebar({ setFilter, setSearch, data }: { setFilter: any; setSearch: any; data: { value: string; done: boolean }[] }) {
...
>>>>>>> dbcd66a1b67cf6154678b8bab05bc3bc849f08b8
  ...
  return (
    <div className="md:w-20 w-full md:h-full bg-[#60c0bf] rounded-2xl border-2 border-black overflow-hidden font-semibold px-3 duration-300 transition-all flex bottom-0 left-0 md:static md:block p-3 md:py-0 absolute" ref={side}>
      <div className="w-full md:flex justify-center items-center py-5 hidden">
        <button className=" text-xl border border-black rounded-xl bg-[#f7cb66] h-12 p-3.5 items-center flex w-12 gap-1 overflow-hidden transition-all duration-300 relative" onClick={() => handleClickMenu()} ref={menuBtn}>
          <AiOutlineMenu />
          <span className="text-xl absolute -translate-x-full w-full flex justify-center items-center opacity-0 duration-300" ref={menu}>
            Menu
          </span>
        </button>
      </div>
      ...
      <div className="flex md:flex-col gap-3 justify-between w-full items-center ">
        <button
          className={`${currentFilter === '' ? 'bg-slate-800/40' : 'bg-[#7fbc95]'} md:w-full p-3.5 h-12 md:h-auto text-xl rounded-xl border border-black flex gap-2 items-center transition-all duration-300 relative overflow-hidden`}
          ref={allBtn}
          onClick={() => {
            setFilter(null);
            setSearch(null);
...
=======
            className={`text-xl md:absolute md:-translate-x-full ${
              (currentFilter === '' && widthPage < 768 && 'static translate-x-0') || (currentFilter !== '' && widthPage < 768 && 'absolute translate-x-full')
>>>>>>> dbcd66a1b67cf6154678b8bab05bc3bc849f08b8
        ...
        <button
          className={`${currentFilter === 'checked' ? 'bg-slate-800/40' : 'bg-[#7fbc95]'} md:w-full p-3.5 h-12 md:h-auto text-xl rounded-xl border border-black flex gap-2 items-center transition-all duration-300 relative overflow-hidden`}
          ref={checkBtn}
          onClick={() => {
        ...
        >
          ...
          <span
            className={`text-xl md:absolute md:-translate-x-full ${
              (currentFilter === 'checked' && widthPage < 768 && 'static translate-x-0') || (currentFilter !== 'checked' && widthPage < 768 && 'absolute translate-x-full')
        ...
        <button
          className={`${currentFilter === 'uncheck' ? 'bg-slate-800/40' : 'bg-[#7fbc95]'} md:w-full ${
        ...
        >
          <AiOutlineClockCircle className={`flex-shrink-0 ${currentFilter === 'uncheck' && widthPage < 768 ? '' : 'ml-7'} md:m-0`} />
          <span
            className={`md:absolute md:opacity-0 w-full ${(currentFilter === 'uncheck' && widthPage < 768 && 'translate-x-0') || (currentFilter !== 'uncheck' && widthPage < 768 && 'translate-x-full')} duration-300`}
            ref={chek}
